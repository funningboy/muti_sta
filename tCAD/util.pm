...
#==================================
# define from tCAD::tTable
#==================================
sub is_cell_module_deep {
    my ($self,$cell) = (@_);

       if( defined($self->{cell}->{$cell}) ) { return 0; }

return -1;
}

sub get_cell_module_delay {
   my ($self,$cell) = (@_);
   if( !defined($self->{cell}->{$cell})){ return (); }
return $self->{cell}->{$cell}->{delay}; 
}

sub get_cell_module_tTable {
   my ($self,$cell,$logic) = (@_);
   if( !defined($self->{cell}->{$cell}->{tTable}->{$logic}) ){ return (); }
return $self->{cell}->{$cell}->{tTable}->{$logic};
}
